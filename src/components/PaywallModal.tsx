...
import { Crown, Zap, Shield, Sparkles, CheckCircle2, Loader2, Lock, Flame, Trophy, Infinity as InfinityIcon } from "lucide-react";
import { STRIPE_CHECKOUT_URL, setPremium, getLockoutSecondsLeft } from "@/lib/premium";
...
export function PaywallModal({ open, onClose, onActivated }: PaywallModalProps) {
  ...
  useEffect(() => {
    if (!open) return;
    setSecondsLeft(getLockoutSecondsLeft());
    const timer = setInterval(() => {
      const s = getLockoutSecondsLeft();
      setSecondsLeft(s);
      if (s <= 0) {
        clearInterval(timer);
        onClose();
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-950/95 backdrop-blur-lg flex items-center justify-center p-4 overflow-y-auto">
      <div className="relative max-w-md w-full my-auto" style={{ animation: "modalIn 400ms ease-out" }}>
        {/* No close button — unclosable */}
        ...
        <div className="bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950 border-2 border-amber-400/40 rounded-3xl p-6 sm:p-8 text-center shadow-2xl shadow-amber-500/20">
          {success ? (
            ...
            <>
              ...
              <h2 className="text-3xl font-black text-white mb-1">You're Hooked!</h2>
              <p className="text-amber-300 font-bold text-sm mb-1">Keep the streak going — go Premium.</p>
              <p className="text-slate-400 text-sm mb-6">You've played 3 games and you're on fire. Don't let your streak die.</p>
              ...
              <div className="mb-6 space-y-2.5 text-left">
                <div className="flex items-center gap-2.5 text-sm text-slate-200 bg-slate-800/50 rounded-xl px-3 py-2.5 border border-slate-700/50">
                  <div className="w-8 h-8 rounded-lg bg-amber-500/20 flex items-center justify-center flex-shrink-0"><Zap className="w-4 h-4 text-amber-400" /></div>
                  <span><b className="text-white">Unlimited gameplay</b> — play as much as you want</span>
                </div>
                <div className="flex items-center gap-2.5 text-sm text-slate-200 bg-slate-800/50 rounded-xl px-3 py-2.5 border border-slate-700/50">
                  ...
                  <span><b className="text-white">Keep your daily streak</b> — never lose progress</span>
                </div>
                <div className="flex items-center gap-2.5 text-sm text-slate-200 bg-slate-800/50 rounded-xl px-3 py-2.5 border border-slate-700/50">
              ...
              <div className="mb-5 inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-green-500/15 border border-green-400/30 text-green-300 font-bold text-xs">
                <CheckCircle2 className="w-3.5 h-3.5" /> Cancel anytime · Instant access
              </div>
              ...
              >
                <span className="flex items-center justify-center gap-2">
                  ...
                  {loading ? "Waiting for payment..." : "SUBSCRIBE & PLAY NOW"}
                </span>
              </button>

              {/* Countdown / wait option */}
              <div className="mt-5 pt-5 border-t border-slate-800">
                <p className="text-slate-500 text-xs mb-2">Or wait for your free session to reset</p>
